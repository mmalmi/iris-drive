...
pub(crate) enum Command {
    ...
    /// `AppKey` on this machine. Single-install default; this `AppKey` starts
    ...
    /// recover an `NostrIdentity` onto this install. A fresh local `AppKey` is
    ...
    /// local `AppKey`; approval arrives through signed `NostrIdentity` roster
    /// ops.
    Link {
    ...
    Logout,
    /// Approve a pending install by adding its `AppKey` to the `NostrIdentity`
    ...
    /// Print the current `NostrIdentity`/`AppKeys` roster projection as JSON.
    Roster,
    ...
    Drives,
    /// Show the local `NostrIdentity` and current `AppKey`.
    Whoami,
...
pub(crate) enum AppKeysCmd {
    ...
    Request {
        /// Invite URL from an admin `AppKey`, or `NostrIdentity` UUID for manual pairing.
